...
static void sem_capsa(SemanticState*, Capsa**);
...
static void sem_definition_capsa(SemanticState*, Definition*);
static void sem_definition_function(SemanticState*, Definition*);
static void sem_definition_method(SemanticState*, Definition*);
static void sem_definition_constructor(SemanticState*, Definition*);
static void sem_definition_structure(SemanticState*, Definition*);
static void sem_definition_monitor(SemanticState*, Definition*);

static void defaultdeffunc(SemanticState*, Definition*);

static void sem_definition(SemanticState* state, Definition* definition) {
    switch (definition->tag) {
    case DEFINITION_CAPSA:
        sem_definition_capsa(state, definition);
        break;
    case DEFINITION_FUNCTION:
        sem_definition_function(state, definition);
        break;
    case DEFINITION_METHOD:
        sem_definition_method(state, definition);
        break;
    case DEFINITION_CONSTRUCTOR:
        sem_definition_constructor(state, definition);
        break;
    case DEFINITION_TYPE:
        switch (definition->type->tag) {
        case TYPE_STRUCTURE:
            sem_definition_structure(state, definition);
            break;
        case TYPE_MONITOR:
            sem_definition_monitor(state, definition);
            break;
        default:
            UNREACHABLE;
        }
...
static void sem_definition_capsa(SemanticState* state, Definition* def) {
    Capsa* capsa = def->capsa.capsa;
    if (!symtable_insert(state->table, def)) {
        err_redeclaration(capsa->id);
    }
    if (capsa->type) { // For non-inferred types
        linktype(state->table, &capsa->type);
    }
    if (def->capsa.expression) {
        sem_expression(state, def->capsa.expression);
...
static void sem_definition_function(SemanticState* state, Definition* def) {
    assert(!def->function.private);
    if (!symtable_insert(state->table, def)) {
        err_redeclaration(def->function.id);
    }
    linktype(state->table, &def->function.type);
    defaultdeffunc(state, def);
}

static void sem_definition_method(SemanticState* state, Definition* def) {
    assert(insidemonitor(state));
        
    // TODO: take line (-1) from definition
    Capsa* capsa = astself(-1);
    capsa->type = ast_type_id(ast_id(
        -1, state->structure->structure.id->name)
    );
...
static void sem_definition_constructor(SemanticState* state, Definition* def) {
    assert(insidestructure(state));
    state->initializer = true;
    def->function.id = state->structure->structure.id;
    def->function.type = state->structure; 
    defaultdeffunc(state, def);
}

static void sem_definition_structure(SemanticState* state, Definition* def) {
    if (!symtable_insert(state->table, def)) {
        err_redeclaration(def->type->structure.id);
    }
    state->structure = def->type;
    symtable_enter_scope(state->table);
...
static void sem_definition_monitor(SemanticState* state, Definition* def) {
    if (!symtable_insert(state->table, def)) {
        err_redeclaration(def->type->structure.id);
    }
    state->structure = def->type;
...
// auxiliary
static void defaultdeffunc(SemanticState* state, Definition* def) {
    // parameters
    symtable_enter_scope(state->table);
    FOREACH(Definition, p, def->function.parameters) {
        sem_definition(state, p);
        if (insidemonitor(state) && !safetype(p->capsa.capsa->type)) {
            TODOERR(
                p->capsa.capsa->line,
    ...
    // block
    state->return_ = def->function.type;
    sem_block(state, def->function.block);
    state->return_ = NULL;
    symtable_leave_scope(state->table);
...
static void sem_statement(SemanticState* state, Statement* statement) {
    ...
    case STATEMENT_ASSIGNMENT:
        sem_capsa(state, &statement->assignment.capsa);
        if (statement->assignment.capsa->value) { // can't be reassigned
            err_assignment_value(statement);
        }
        if (statement->assignment.capsa->tag == CAPSA_INDEXED &&
            statement->assignment.capsa->indexed.array->type->immutable) {
            TODOERR(statement->line, "can't assign to immutable arrays");
        }
        sem_expression(state, statement->assignment.expression);
        assignment(statement->assignment.capsa,
...
static void sem_capsa(SemanticState* state, Capsa** capsa_pointer) {
